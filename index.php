<?php
    ...
    $query = "SELECT id, title, description, createdOn FROM posts ORDER BY createdOn DESC LIMIT 12";

    if(isset($_REQUEST["page"]) && is_numeric($_REQUEST["page"])) {
        $page = $_GET["page"];
        if($page > 1 && $page - 1 <= floor($count / 12)) {
            $query .= " OFFSET " . (($page - 1) * 12);
        }
    }
